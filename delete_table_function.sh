...
AS \$function\$
BEGIN
  -- Delete the table from the specified schema
  EXECUTE format('DROP TABLE IF EXISTS %I.%I;', schema_name_in, table_name_in);
...
# Drop any existing function called delete_table before executing the SQL command
psql -U "$PG_USER" -h $PG_HOST -p $PG_PORT -d "$DB_NAME" -c "DROP FUNCTION IF EXISTS public.delete_table(text, text);"
